...
import { GameStatus, default as GameType, Player } from "@/lib/models/game";
import { RootStateType } from "@/reducers";
import { GetGameState, getGame } from "@/reducers/gameReducer";
...
export default function Game() {
  ...
  const getGameState: GetGameState = useSelector<RootStateType, GetGameState>((state) => state.game.getGame);
  ...
  useEffect(() => {
    if (getGameState.game?.id) {
      setWsUrl(import.meta.env.PUB_API_BASE.replace("http://", "ws://").replace("https://", "wss://") + "/play/" + getGameState.game?.id)
      setCurrentGame(getGameState.game)
    }
  }, [getGameState]);
  ...
  return (
    <>
      {getGameState.inProgress && <LoadingSpinner size={64} />}
      {getGameState.game &&
          ...
          <div className="basis-52 bg-slate-300/[0.4] mr-5 rounded-sm p-2 items-center text-left text-gray-300 font-bold">
            <div>
              ...
              <div className="flex items-center">
                ...
                <div className={clsx(
                  ...
                  currentGame?.gameState.playerTurn === Player.BLACK_PLAYER ? "bg-black" : "bg-white"
                )}></div>
              </div>
              <div className="flex items-center text-rose-500 justify-center mt-2 min-h-2">
                {
                  currentGame?.gameState.gameStatus === GameStatus.CHECKMATE
                    ? "Check mate!"
                    : currentGame?.gameState.gameStatus === GameStatus.STALEMATE
                      ? "Stalemate!"
                      : currentGame?.gameState.checkedPlayer !== Player.UNKNOWN_PLAYER
                        ? "Check"
                        : ""
                }
              </div>
            ...
            <div className="mt-10">
              ...
              <div className="flex bg-black text-white min-h-12 px-2 py-1 items-start">
                {currentGame?.gameState.outTable.filter(p => p.player === Player.WHITE_PLAYER).map(p => pieceTypeToChar(p.pieceType))}
              </div>
              <div className="flex bg-white text-black min-h-12 px-2 py-1 items-start">
                {currentGame?.gameState.outTable.filter(p => p.player === Player.BLACK_PLAYER).map(p => pieceTypeToChar(p.pieceType))}
              </div>
            </div>
            <div className="mt-10">
              ...
              <div className="flex text-sm">
                {currentGame?.gameState.moves.map(m => m + " ")}
              </div>
            </div>
          </div>
          <div className="basis-auto shrink">
            <GameTable gameRelation={gameRelation} initialGame={getGameState.game} ref={gameTableRef} onMove={onTableMove} onGameUpdate={(updatedGame) => setCurrentGame(updatedGame)} />
          </div>
        </div>
      }
    </>
